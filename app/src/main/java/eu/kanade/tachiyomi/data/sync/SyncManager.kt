...
class SyncManager(
    ...
    suspend fun syncData() {
        ...
        logcat { "Got Backup Uri: $backupUri" }
        if (backupUri != null) {
    ...
    private fun writeSyncDataToCache(context: Context, backup: Backup): Uri? {
        val cacheFile = File(context.cacheDir, "mihon_sync_data.proto.gz")
        return try {
            ...
            logcat { "Failed to write sync data to cache: ${e.message}" }
            null
        }
    }
    ...
    private suspend fun filterFavoritesAndNonFavorites(backup: Backup): Pair<List<BackupManga>, List<BackupManga>> {
        ...
        val elapsedTimeMillis = measureTimeMillis {
            ...
            logcat { "Starting to filter favorites and non-favorites from backup data." }
                ...
                when {
                    ...
                    remoteManga.favorite -> {
                        if (localManga == null || isMangaDifferent(localManga, remoteManga)) {
                            logcat { "Adding to favorites: ${remoteManga.title}" }
                            favorites.add(remoteManga)
                        } else {
                            logcat { "Already up-to-date favorite: ${remoteManga.title}" }
                        }
                    }
                    // Handle non-favorites
                    !remoteManga.favorite -> {
                        logcat { "Adding to non-favorites: ${remoteManga.title}" }
        ...
        nonFavorites.forEach { nonFavorite ->
            ...
            localMangaMap[key]?.let { localManga ->
                if (localManga.favorite != nonFavorite.favorite) {
                    updateManga.await(MangaUpdate(id = localManga.id, favorite = nonFavorite.favorite))
                }
            }
        }
    }
